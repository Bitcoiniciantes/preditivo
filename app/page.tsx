...
import { subscribeLivePrice, type LivePriceStatus } from "../lib/live-price";
import { defaults, displayAsset, marketSymbols, rsiLabelByPeriod, staticAssets } from "../lib/config";
import type { BiasItem, MarketData, MultiRsi, NuplReading, Signal } from "../lib/types";
...
const fallbackSignals:Signal[] = [
  {
    ...
    score:0,
    group:"Dados",
    detail:"Os sinais serão calculados quando os candles reais forem carregados.",
  },
];
function PeriodScoreStrip({periods,scores,active}:{periods:readonly string[];scores:Record<string,number|null>;active:string}){return <div className="periodScoreStrip" aria-label="Notas do radar por período">{periods.map(itemPeriod=>{const value=scores[itemPeriod],tone=value===null||value===undefined?"loading":value>=20?"buy":value<=-20?"sell":"neutral";return <span key={`score-${itemPeriod}`} className={`${tone} ${active===itemPeriod?"active":""}`} title={`${itemPeriod}: ${value===null||value===undefined?"carregando":`${value>0?"+":""}${value}`}`}>{value===null||value===undefined?"…":`${value>0?"+":""}${value}`}</span>})}</div>}
...
function readCachedMarket(asset:string,period:string):MarketData|null{try{const saved=localStorage.getItem(`termometro-market-${asset}-${period}`),cached:MarketData|null=saved?JSON.parse(saved):null;return cached&&cached.asset===asset&&cached.candles.length>=55?{...cached,period}:null}catch{return null}}
function marketErrorMessage(error:unknown,asset:string){return error instanceof Error&&error.message?error.message:staticAssets[asset]?`Dados de ${displayAsset(asset)} temporariamente indisponíveis`:"Ativo indisponível ou bloqueado pela fonte"}
export function Termometro(){
 const [ticker,setTicker]=useState("BTC"),[query,setQuery]=useState(""),[period,setPeriod]=useState("1H"),[open,setOpen]=useState<number|null>(null),[assets,setAssets]=useState<string[]>(()=>{if(typeof window==="undefined")return defaults;try{const stored:string[]|null=JSON.parse(localStorage.getItem("termometro-assets")||"null");return stored?[...new Set([...defaults,...stored.filter(asset=>asset!=="SOL")])]:defaults}catch{return defaults}});
 ...
 const [quickChanges,setQuickChanges]=useState<Record<string,number>>({});
 ...
 useEffect(()=>{let active=true;async function load(){try{const data=await fetchBitcoinNupl();if(active)setNupl(data)}catch{if(active)setNupl(null)}}load();const timer=window.setInterval(load,600000);return()=>{active=false;window.clearInterval(timer)}},[]);
 const [previousReading,setPreviousReading]=useState<{period:string;score:number}|null>(null);
 const loadMarket=(asset:string,period:string,signal?:AbortSignal)=>{if(staticAssets[asset])return fetchMarket(asset,period,signal);return fetchMarket(asset,period,signal).catch(()=>fetchLiveStockMarket(asset,period,signal))};
 useEffect(()=>{const controller=new AbortController();setPrice24hReference(null);loadMarket(ticker,"1H",controller.signal).then(data=>{const reference=data.candles.at(-25)?.close;if(!controller.signal.aborted&&reference&&reference>0)setPrice24hReference({asset:ticker,price:reference})}).catch(()=>{if(!controller.signal.aborted)setPrice24hReference(null)});return()=>controller.abort()},[ticker]);
 useEffect(()=>{if(!liveEligible||liveStatus==="live")return;const controller=new AbortController();const refresh=()=>{loadMarket(ticker,period,controller.signal).then(data=>{if(!controller.signal.aborted)setMarket(data)}).catch(()=>{})};refresh();const timer=window.setInterval(refresh,15000);return()=>{controller.abort();window.clearInterval(timer)}},[ticker,period,liveEligible,liveStatus]);
 useEffect(()=>{const controller=new AbortController();loadMarket(ticker,'1H',controller.signal).then(data=>{const reading=analyze(data);if(reading&&!controller.signal.aborted)setPreviousReading({period:'1H',score:reading.score})}).catch(()=>{if(!controller.signal.aborted)setPreviousReading(null)});return()=>controller.abort()},[ticker]);
 const [usingCached,setUsingCached]=useState(false);
 useEffect(()=>{if(!market||market.asset!==ticker||usingCached)return;try{localStorage.setItem(`termometro-market-${ticker}-${period}`,JSON.stringify(market))}catch{}},[market,ticker,period,usingCached]);
 const retryMarket=()=>{setLoading(true);setMarketError('');setUsingCached(false);loadMarket(ticker,period).then(data=>{setMarket(data);setMarketError('')}).catch(error=>{const cached=readCachedMarket(ticker,period);setMarket(cached);setUsingCached(!!cached);setMarketError(marketErrorMessage(error,ticker))}).finally(()=>setLoading(false))};
 const [periodFeedback,setPeriodFeedback]=useState(""),[showPeriodHelp,setShowPeriodHelp]=useState(false);
 useEffect(()=>{const controller=new AbortController();loadMarket(ticker,period,controller.signal).then(data=>{if(controller.signal.aborted)return;setMarket(data);setMarketError('');setUsingCached(false);setPeriodFeedback(current=>current?("✓ Termômetro atualizado para "+period):"")}).catch(error=>{if(controller.signal.aborted)return;const cached=readCachedMarket(ticker,period);setMarket(cached);setUsingCached(!!cached);setMarketError(marketErrorMessage(error,ticker))}).finally(()=>{if(!controller.signal.aborted)setLoading(false)});return()=>controller.abort()},[ticker,period]);
 useEffect(()=>{const controller=new AbortController();let fetching=false;const refresh=async()=>{if(fetching)return;fetching=true;try{const data=await fetchMultiRsi(ticker,controller.signal);if(!controller.signal.aborted)setMultiRsi(data)}catch{if(!controller.signal.aborted)setMultiRsi(null)}finally{fetching=false;if(!controller.signal.aborted)setMultiRsiLoading(false)}};refresh();const timer=window.setInterval(refresh,30000);return()=>{controller.abort();window.clearInterval(timer)}},[ticker]);
 ...
 useEffect(()=>{let active=true;const load=async()=>{const next:Record<string,number>={};const results=await Promise.all(assets.map(async asset=>{try{const response=await fetch(`https://data-api.binance.vision/api/v3/ticker/24hr?symbol=${asset}USDT`,{cache:"no-store"});const quote=await response.json();const change=Number(quote.priceChangePercent);return Number.isFinite(change)?[asset,change] as const:null}catch{return null}}));for(const item of results)if(item)next[item[0]]=item[1];const pending=assets.filter(asset=>!(asset in next));if(pending.length){const external=await Promise.all(pending.map(async asset=>{try{const symbol=marketSymbols[asset]??asset;const response=await fetch(`https://bitcoiniciantes-ia.bitcoiniciantes.workers.dev/api/candles?asset=${encodeURIComponent(symbol)}&period=1H`,{cache:"no-store"});const body=await response.json(),candles=Array.isArray(body?.candles)?body.candles:[],last=Number(candles.at(-1)?.close),reference=Number(candles.at(-25)?.close);return Number.isFinite(last)&&Number.isFinite(reference)&&reference>0?[asset,(last/reference-1)*100] as const:null}catch{return null}}));for(const item of external)if(item)next[item[0]]=item[1]}if(active)setQuickChanges(next)};load();const timer=window.setInterval(load,30000);return()=>{active=false;window.clearInterval(timer)}},[assets]); useEffect(()=>{const controller=new AbortController();setPeriodScoreState({asset:ticker,scores:{}});mapSettledWithConcurrency(periodOptions,3,(itemPeriod)=>loadMarket(ticker,itemPeriod,controller.signal)).then(results=>{if(controller.signal.aborted)return;setPeriodScoreState({asset:ticker,scores:Object.fromEntries(results.map((result,index)=>{if(result.status!=="fulfilled")return[periodOptions[index],null];const reading=analyze(result.value);return[periodOptions[index],reading?.score??null]}))})});return()=>controller.abort()},[ticker]);
 ...
 const selectAsset=(asset:string,nextPeriod="1D")=>{notifyParentAiContext(asset,nextPeriod);setLoading(true);setMarketError("");setUsingCached(false);setPreviousReading(null);if(asset!==ticker)setMultiRsiLoading(true);setPeriod(nextPeriod);setTicker(asset)};
 ...
 const confluence=useMemo(()=>buildConfluence(market,analysis,multiRsi),[market,analysis,multiRsi]); const loadedPeriodScores=periodScoreState.asset===ticker?periodScoreState.scores:{}; const currentPeriodScore=market?.asset===ticker&&market.period===period?analysis?.score:null; const displayedPeriodScores={...loadedPeriodScores,[period]:currentPeriodScore??loadedPeriodScores[period]}; const extreme=analysis?.extreme; const signals=analysis?.signals??fallbackSignals,score=analysis?.score??0,confidence=analysis?.confidence??0,change=analysis?.change??0;
 const livePrice=liveQuote?.asset===ticker?liveQuote.price:undefined;
 const currentPrice=livePrice??market?.candles.at(-1)?.close;
 const change24h=currentPrice&&price24hReference?.asset===ticker&&price24hReference.price>0?(currentPrice/price24hReference.price-1)*100:null;
 const chartCandles=useMemo(()=>{if(!market||!livePrice||staticAssets[ticker])return market?.candles??[];const candles=[...market.candles],last=candles.at(-1);if(!last)return candles;candles[candles.length-1]={...last,close:livePrice,high:Math.max(last.high,livePrice),low:Math.min(last.low,livePrice)};return candles},[market,livePrice,ticker]);
 ...
 const patternTitle=analysis&&signals[1]?.score>0?signals[1].summary:"Sem padrão confirmado";
 ...
 const radarItems=useMemo(()=>[...ranking.filter(item=>item.asset!=="MSTR").map(item=>({...item,available:true})),...assets.filter(asset=>asset!=="MSTR"&&!ranking.some(item=>item.asset===asset)).map(asset=>({asset,score:0,confidence:0,change:0,rsi:undefined,available:false}))],[ranking,assets]);
 const availablePeriods=periodOptions;
 ...
 return <MarketStreamProvider><main>
  ...
  <div className="analysisColumn analysisLeft">
    ...
    <div className="deskTop">
      <div className="deskAsset"><button type="button" className="assetIcon assetAiButton" onClick={() => { if (window.parent !== window) { window.parent.postMessage({ type: "termometro:open-estudebitcoin-ai", asset: ticker, period, marketData: aiMarketData }, "https://bitcoiniciantes.github.io"); return; } setAiRunKey(Date.now()); window.setTimeout(() => document.getElementById("analista-digital")?.scrollIntoView({ behavior: "smooth", block: "start" }), 30); }} disabled={!analysis || loading || !currentPrice} aria-label="Abrir Analista Digital e gerar leitura">IA</button><div><span className="eyebrow">ATIVO EM ANÁLISE</span><h1>{displayName}<small>/{currency}</small></h1></div></div>
      <div className={`deskQuote ${cryptoLive?"quoteLive":""}`}><span>{cryptoLive?"COTAÇÃO AO VIVO":"ÚLTIMO PREÇO"}</span><b>{fmt(currentPrice)}</b><small className={change24h!==null&&change24h<0?"down":""}>{change24h===null?"24H INDISPONIVEL":`${change24h>=0?"+":""}${change24h.toFixed(2)}% EM 24H`}</small><small className={change<0?"down":""}>{analysis?`${change>=0?"+":""}${change.toFixed(2)}% NO CANDLE ${period}`:marketError||"Carregando..."}</small></div>
    ...
    <div className="desktopSearchRow">
    ...
    <div className={`rsiMirror ${selectedRsiTone}`} role="status" aria-live="polite"><span>RSI</span><b>{selectedRsi?selectedRsi.value.toLocaleString("pt-BR",{minimumFractionDigits:1,maximumFractionDigits:1}):"—"}</b><em>{selectedRsiStatus}</em></div>
...
<RadarBoard items={radarItems} changes={quickChanges} ticker={ticker} onSelect={selectAsset} onRemove={removeAsset} onAi={()=>{if(window.parent!==window){window.parent.postMessage({type:"termometro:open-estudebitcoin-ai",asset:ticker,period,marketData:aiMarketData},"https://bitcoiniciantes.github.io");return}setAiRunKey(Date.now());window.setTimeout(()=>document.getElementById("analista-digital")?.scrollIntoView({behavior:"smooth",block:"start"}),30)}} />
  </div><article className="card chart" id="grafico"><div className="cardTitle chartTitle"><div><span>ESTRUTURA DE PREÇO</span><b>{patternTitle}</b></div>{ticker==="BTC"&&nupl&&<NuplBar nupl={nupl}/>}<div className="chartPeriodSwitch" aria-label="Período do gráfico"><span>TIME FRAME</span><div>{availablePeriods.map(p=><button key={`chart-${p}`} type="button" onClick={()=>changePeriod(p)} className={period===p?"active":""} aria-pressed={period===p}>{p}</button>)}</div></div></div><PriceStructureChart asset={displayName} candles={chartCandles} currentPrice={currentPrice ?? 0} currency={currency} loading={loading} period={period} resistance={resistance} support={support}/><div className="chartFoot"><span><i className="dot candleUpDot"/>Alta: fechamento ≥ abertura</span><span><i className="dot candleDownDot"/>Baixa: fechamento &lt; abertura</span><span>Volume <b>{volumeRatio?`${volumeRatio.toFixed(2)}× média`:"—"}</b></span><span className="chartPurpose">Corpo: abertura–fechamento · Pavio: mínima–máxima · Níveis calculados com candles concluídos.</span></div></article>
    <RadarBoard items={radarItems} changes={quickChanges} ticker={ticker} onSelect={selectAsset} onRemove={removeAsset} compact />
...
function RadarBoard({items,changes,ticker,onSelect,onRemove,onAi,compact=false}:{items:RadarItem[];changes:Record<string,number>;ticker:string;onSelect:(asset:string,period?:string)=>void;onRemove:(asset:string)=>void;onAi?:()=>void;compact?:boolean}){
  if(compact)return <div className="biasBoard chartRadar"><div className="quickAssetList">{items.map(item=>{const change=changes[item.asset],tone=change===undefined?"flat":change>0?"up":"down";return <button type="button" key={item.asset} className={`quickAsset ${tone} ${ticker===item.asset?"selected":""}`} onClick={()=>onSelect(item.asset,"15M")} aria-pressed={ticker===item.asset}><b>{displayAsset(item.asset)}</b><em>{change===undefined?"—":`${change.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}%`}</em></button>})}</div></div>;
  return <div className="biasBoard"><div className="biasHead"><span>RADAR COMPRA × VENDA</span>{onAi&&<button type="button" className="biasAiButton" onClick={onAi} aria-label={`Análise com IA para ${displayAsset(ticker)}`} title={`Análise com IA: ${displayAsset(ticker)}`}><span aria-hidden="true">✦</span> Análise com IA</button>}</div><div className="biasList">{items.map((item)=>{const side=!item.available?"SEM DADOS":item.score>=20?"COMPRA":item.score<=-20?"VENDA":item.score>=10?"NEUTRO ↑":item.score<=-10?"NEUTRO ↓":"NEUTRO",sideTone=!item.available?"sem-dados":item.score>=20?"compra":item.score<=-20?"venda":item.score>=10?"neutro-alta":item.score<=-10?"neutro-baixa":"neutro",toneColor=!item.available?"#718079":item.score>=20?"#54b85a":item.score<=-20?"#e2555d":"#c39a45",ringDegrees=(Math.max(-100,Math.min(100,item.score))+100)*1.8;return <div key={item.asset} className={`biasItem ${sideTone} ${ticker===item.asset?"selected":""}`}><button type="button" className="biasSelect" onClick={()=>onSelect(item.asset)} aria-pressed={ticker===item.asset}><span className="biasRsi">RSI=<b className={item.rsi===undefined?"neutral":item.rsi>=55?"high":item.rsi<=45?"low":"neutral"}>{item.rsi===undefined?"—":Math.round(item.rsi)}</b></span><span className="biasGauge" style={{background:`conic-gradient(from 180deg,${toneColor} ${ringDegrees}deg,#5a5f66 ${ringDegrees}deg 360deg)`}}><strong>{item.available?`${item.score>0?"+":""}${item.score}`:"—"}</strong><b>{displayAsset(item.asset)}</b></span><span className="biasSide">{side.replace(/[↑↓]/u,"")}<em>{side.includes("↑")?"↑":side.includes("↓")?"↓":""}</em></span></button>{!defaults.includes(item.asset)&&<button className="biasRemove" onClick={()=>onRemove(item.asset)} aria-label={`Remover ${displayAsset(item.asset)}`} title={`Remover ${displayAsset(item.asset)}`}>×</button>}</div>})}</div><p>Viés técnico comparativo; não representa ordem de entrada.</p></div>
...
function RsiGeneralPanel({asset,data,loading}:{asset:string;data:MultiRsi|null;loading:boolean}){const status=(v:number)=>v>=80?"Sobrecompra":v<=25?"Sobrevenda":v>=55?"Alta":v<=45?"Baixa":"Neutro",tone=(v:number)=>v>=55?"high":v<=45?"low":"neutral",gauge=(v:number)=>"■".repeat(Math.round(v/10))+"□".repeat(10-Math.round(v/10)),rows=data?[...data.rows,{label:"GERAL",value:data.general,previous:data.previousGeneral}]:[];return <article className="rsiGeneral card" aria-busy={loading}><div className="rsiTitle">{asset} <span>•</span> RSI GERAL PRO</div><div className="rsiGrid rsiHeader"><span>TF</span><span>RSI</span><span>LEITURA</span><span>DIR.</span></div>{loading?<div className="rsiEmpty">CALCULANDO 6 PERÍODOS…</div>:!data?<div className="rsiEmpty">DADOS MULTI-TEMPORAIS INDISPONÍVEIS</div>:<>{rows.map(row=><div className={`rsiGrid rsiRow ${row.label==="GERAL"?"general":""}`} key={row.label}><b>{row.label}</b><strong className={tone(row.value)}>{row.value.toFixed(1)}</strong><span className={tone(row.value)}>{status(row.value)}</span><i className={row.value>=row.previous?"up":"down"}>{row.value>=row.previous?"▲":"▼"}</i></div>)}<div className="rsiGrid consensus"><b>CONSENSO</b><strong>{data.bullCount}▲</strong><span>{Math.round(data.bullCount/data.rows.length*100)}% alta</span><i>{data.bearCount}▼</i></div><div className={`rsiSummary ${tone(data.general)}`}><b>SINAL</b><span>{data.signal}</span></div><div className={`rsiSummary force ${tone(data.general)}`}><b>FORÇA</b><span>{gauge(data.general)}  {data.general.toFixed(1)}</span></div><small>◉ RSI AO VIVO • candle atual • atualização a cada 30s</small></>}</article>}export default function Home(){return <Termometro/>}
