...
export default function PositionPanel({ asset, currentPrice, assets }: { asset: string; currentPrice?: number; assets: string[] }) {
  ...
  const addPosition = () => {
    ...
    setPositions((current) => [...current, { id: positionAsset + "-" + Date.now() + "-" + Math.random().toString(36).slice(2, 8), asset: positionAsset, side, averagePrice: entry, quantity: size }]);
    setAveragePrice(""); setQuantity("");
  };

  return <article className="card positionPanel">
    <div className="cardTitle positionTitle"><div><span>{"POSI\u00c7\u00d5ES EM ABERTO \u2022 TODOS OS ATIVOS"}</span><b>{"Controle de compra e venda"}</b></div><span className="positionQuote">{positions.length + " POSI\u00c7\u00c3O(\u00d5ES)"}</span></div>
